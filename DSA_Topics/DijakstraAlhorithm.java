...
public class DijakstraAlhorithm{
    ...
    public static void dijkstra(int[][] graph, int source){
        ...
        for (int i = 0; i < numVertices; i++){
            ...
            for (int j = 0; i < numVertices; j++){
                if (!visited[j] && graph[minVertex][j] != 0 && distances[minVertex] != INF && distances[minVertex] + graph[minVertex][j] < distances[j]){
    ...
    private static void printDistances(int[] distances, int source){
        System.out.println("Shortest distnaces from vertex " + source + ":");
        for (int i = 0; i < distances.length; i++){
